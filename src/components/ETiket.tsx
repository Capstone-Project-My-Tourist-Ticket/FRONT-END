    ...
    <div className="px-4 py-2 text-gray-800">
      <div className="hidden xl:flex flex-row justify-between shadow-md border rounded-md">
        ...
        <div className="relative flex flex-col w-3/4">
          <img className="h-[335px]" src={bookingDetail?.tour.image} />
            ...
            <div className="absolute flex flex-row p-2 text-gray-800 opacity-100">
              ...
              <div className="flex flex-col ml-4">
                <div className="hidden md:flex flex-col">
                  ...
                  <p className="text-xs font-semibold text-red-800">
                    {" "}
                    {bookingDetail?.tour.address.toUpperCase()}
                  </p>
                </div>
                <div className="flex flex-col mt-1">
                  <p className="mb-1 text-xs text-gray-500">Quantity</p>
                  <p className="text-xs font-semibold text-red-800">
                    {bookingDetail?.quantity} PAX
                  </p>
                </div>
              </div>
              <div className="flex flex-col ml-4">
                <div className="flex flex-col">
                  ...
                  <p className="text-xs font-semibold text-red-800">
                    {bookingDetail?.greeting} {bookingDetail?.full_name?.toLocaleUpperCase()}
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="xl:hidden flex flex-col bg-white border rounded-md shadow-md">
        ...
        <img src={bookingDetail?.tour.image} className="h-64" />
        <div className="py-2 px-4 flex flex-col text-sm md:text-2xl">
          <div className="flex text-sm justify-between my-2 md:text-xl">
            <p>Tour :</p>
            <p className="font-bold text-red-800">{bookingDetail?.tour.tour_name.toUpperCase()}</p>
          </div>
          <div className="flex text-sm justify-between my-2 md:text-xl">
            <p>Package :</p>
        ...
        <div className="py-2 px-4 flex flex-col text-sm md:text-2xl">
          <div className="flex text-sm md:text-xl justify-between my-2">
            <p>Pax</p>
            <p className="font-bold text-red-800">{bookingDetail?.quantity}</p>
          </div>
          <div className="flex text-sm md:text-xl justify-between my-2">
            <p>Valid til</p>
            <p className="font-bold text-red-800">{bookingDetail?.booking_date}</p>
          </div>
        </div>
        <div className="py-2 px-4 flex flex-col text-sm md:text-2xl">
          ...
          <div className="flex text-sm md:text-xl justify-between my-2">
            ...
            <p className="font-bold text-red-800">{bookingDetail?.tour.address.toUpperCase()}</p>
          </div>
        </div>
