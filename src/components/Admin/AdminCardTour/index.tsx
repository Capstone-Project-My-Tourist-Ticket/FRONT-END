...
const AdminCardTour = (props: Props) => {
  const { data } = props
  return (
    <Card className="mb-8">
      <CardContent className="p-1">
        <div className="text-3xl font-semibold">
          <Link to={`/admin-tour/${data.id}`}>
          ...
          <div className="flex-col p-2 space-y-1">
            ...
            <div className="flex gap-2 items-center text-sm ">
              ...
              <p className="text-slate-500">
                {data.city.city_name.charAt(0).toUpperCase() +
                  data.city.city_name.slice(1)}
              </p>
            </div>
          </div>
          <p className="p-4 text text-lg text text-red-500 font-semibold">
            {formattedAmount(data.package.price)}
          </p>
        </div>
      </CardContent>
    </Card>
